...
name: StockRush
domain: Backend
eyebrow: Event-driven commerce
summary: 한정 재고 주문의 부분 실패와 중복 처리를 제어하고 주문·재고·결제·조회 모델의 최종 상태 수렴을 장애 시나리오로 검증했습니다.
period: "2026"
role: 개인 프로젝트 · 도메인 모델, 백엔드 서비스, Gateway, 로컬 인프라와 검증 도구 직접 설계·구현
stack:
  - Java
  - Spring Boot
  - Kafka
...
responsibilities:
  - Gateway, Catalog, Inventory, Order, Payment, Promotion, Fulfillment, Read Model의 서비스 경계를 설계했습니다.
  - 주문 Saga, 서비스별 Outbox relay, 소비자 멱등 처리와 관리자 재처리 경로를 구현했습니다.
  - Gateway OIDC/JWT와 서비스 내부 고객 소유권 검사를 분리해 신뢰 경계를 고정했습니다.
flow:
  normal:
    - Gateway 인증
    - 주문·Outbox 저장
  ...
  failure:
    ...
    - 취소 뒤 늦은 결제 승인
  recovery:
    - Processed event 중복 차단
    - Relay 재시도·FAILED 전이
    - 종료 상태 SQL guard
signals:
  - label: 멱등 소비
    expression: event ×2 → outbox 1
    result: 중복 후속 이벤트 0건
    tone: success
...
decisions:
  - title: Saga orchestration
    ...
    alternative: 각 서비스가 다음 이벤트를 직접 연쇄 발행하는 choreography
    reason: 보상 순서와 최종 주문 상태를 한 경계에서 추적하고 운영자가 재처리할 위치를 명확히 하기 위해 선택했습니다.
  - title: Transactional Outbox
    choice: 업무 데이터와 Outbox row를 같은 DB 트랜잭션에 저장합니다.
    alternative: DB commit 이후 애플리케이션에서 Kafka에 직접 발행
    reason: commit과 publish 사이의 실패를 기록으로 남기고 relay가 재시도할 수 있도록 했습니다.
  - title: 이중 신뢰 경계
    choice: Gateway 인증과 서비스 내부 리소스 소유권 검사를 분리합니다.
    alternative: Gateway 검증 결과만 신뢰
...
codeEvidence:
  ...
  - symbol: OutboxRelayService.claimPending
    ...
    proves: 재시도 가능 이벤트만 잠금 선점해 여러 relay의 중복 발행을 피하고 소진 실패를 FAILED로 남깁니다.
...
verification:
  ...
  - layer: integration
    method: PostgreSQL 통합 테스트에서 실패하는 publisher를 주입해 Outbox relay 재시도 횟수를 소진합니다.
